...
# Define the cut-off percentages
CutOff10th = 70
CutOff12th = 70
CutOffBTechCGPA = 6
CutOffBTechPercentage = 60
CutOffLiveKT = 0
CutOffDrop = 0
CutOffGap = 2

# Instantiate the ExcelProcessor class
processor = ExcelProcessor('Wua.xlsx', CutOff10th, CutOff12th, CutOffBTechCGPA,
                           CutOffLiveKT, CutOffDrop, CutOffGap)

# Clean the data
processor.clean_data()

# Check eligibility and format the data
removed_data = processor.check_data()
processor.remove_columns()
processor.basic_editing()

# Assuming user input for whether to keep percentage or CGPA
ask_percentage = True  # Set to False if CGPA is to be kept
processor.remove_unwanted_data(ask_percentage)

# Sort the columns based on a preferred order
preferred_order = [
...
# Rename sheet to RAIT
processor.rename_sheet('RAIT')
processor.fill_empty_cells_with_na()

# Save the cleaned data and removed data to a new Excel file
processor.save_data('processed_file.xlsx', removed_data)

# Adjust column widths based on the length of the longest text in each column for all cells
processor.adjust_column_widths('processed_file.xlsx', 'processed_with_widths.xlsx')
